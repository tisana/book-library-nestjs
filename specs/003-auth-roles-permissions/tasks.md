...
- [ ] T027 [P] Extend SecurityActivityEvent with unique eventId plus foundational identifier-conflict-resolved and identifier-repair-resumed event types in src/auth/schemas/security-activity-event.schema.ts
- [ ] T028 [P] Add conditionally required active-family currentTokenHash and lastRotationOperationId plus its sparse lookup index to RefreshTokenFamily, and add RefreshTokenReplayMarker with globally unique tokenHash and rotationOperationId, familyId/expiry lookup, pending/committed status, 30-second leaseExpiresAt, committedAt, status/lease reconciliation index, family-aligned TTL, and no raw credential/origin/account data in src/auth/schemas/refresh-token-family.schema.ts and src/auth/schemas/refresh-token-replay-marker.schema.ts

...
- [x] T025 Add AuthIdentifierOperation schema with explicit operation/cleanup states, bounded assignments, manifest metadata, deterministic terminalEventId, original/resuming actor fields, partial compound repair-key-policy index, lease indexes, and terminal/cleanup TTL rules in src/auth/schemas/auth-identifier-operation.schema.ts
- [x] T026 Add AuthIdentifierRepairBatch schema after the shared operation assignment shape is available, with bounded assignments, idempotent batch states, checkpoint HMAC metadata, unique parent-operation/batch-number index, parent/status index, and post-cleanup TTL in src/auth/schemas/auth-identifier-repair-batch.schema.ts
- [x] T027 [P] Extend SecurityActivityEvent with unique eventId plus foundational identifier-conflict-resolved and identifier-repair-resumed event types in src/auth/schemas/security-activity-event.schema.ts
- [x] T028 [P] Add conditionally required active-family currentTokenHash and lastRotationOperationId plus its sparse lookup index to RefreshTokenFamily, and add RefreshTokenReplayMarker with globally unique tokenHash and rotationOperationId, familyId/expiry lookup, pending/committed status, 30-second leaseExpiresAt, committedAt, status/lease reconciliation index, family-aligned TTL, and no raw credential/origin/account data in src/auth/schemas/refresh-token-family.schema.ts and src/auth/schemas/refresh-token-replay-marker.schema.ts
- [x] T029 [P] Add unit tests for transaction/saga paths, valid state transitions, failed-retryable reconciliation, finalizing audit-before-TTL crash boundaries, duplicate terminal-event prevention, completed/failed-terminal replay with original HTTP status, reservation-reference attachment after crashes, operation idempotency within retention, assignment limits 19/20/21, pending/gated denial, compensation, and result replay in src/auth/auth-identifier.service.spec.ts
- [x] T030 Implement transactional mutations and durable AuthIdentifierOperation saga coordination using explicit recoverable/finalizing/terminal states, pending and activation-gated reservation references, cleanup status, terminal replay, assignment bounds, compensation, and account aggregate updates in src/auth/auth-identifier.service.ts
- [x] T031 Implement foundational idempotent terminal-event and identifier-repair-resumed writes before TTL eligibility with deterministic eventId and redacted original/resuming actor context in src/auth/security-activity.service.ts
- [x] T032 [P] Add migration tests for identifier/operation/repair-batch/terminal-event/throttle-bucket/replay-marker indexes and TTLs, partial repair-key-policy and unexpired throttle key-version indexes with explain assertions that required-version queries avoid COLLSCAN, unique parent-batch/throttle/replay-hash/replay-operation constraints, replay-marker status/lease lookup, revocation of every active legacy family with security-upgrade-reauth before upgraded refresh handling, removal of legacy current/previous hashes without replay-marker backfill, active-only current-hash validation, absolute-expiry/audit-history preservation, irreversible session revocation on rollback, clean backfill, oversized-conflict quarantine, migration-required readiness, safe identifier rollback, and ephemeral throttle no-backfill behavior in test/auth-identifier-migration.e2e-spec.ts
- [x] T033 Extend the auth migration to create identifier/operation/repair-batch/terminal-event/throttle-bucket/replay-marker indexes and TTLs, including partial compound repair-key-policy and unexpired throttle key-version indexes, unique parent-operation/batch-number checkpoints, unique throttle dimension/keyVersion/bucketKey plus replay tokenHash/rotationOperationId indexes, replay-marker status/lease reconciliation, and activation-gate lookup; before upgraded refresh handling, revoke every active legacy family with security-upgrade-reauth, set revokedAt, clear legacy current/previous hashes from all legacy families without synthesizing partial replay markers, preserve absolute expiry and audit history, and never reactivate legacy sessions on rollback; then backfill reservations, create no throttle history, quarantine oversized identifier conflicts, block/report ambiguity, and preserve history on identifier rollback in migrations/versions/003-auth-roles-permissions.ts
- [x] T034 Register migrated AuthIdentifier/AuthIdentifierOperation/AuthIdentifierRepairBatch schemas without runtime index creation in src/auth/auth.module.ts
- [x] T035 Add `@nestjs/schedule` dependency and ScheduleModule wiring in package.json, src/app.module.ts, and src/auth/auth.module.ts
- [x] T036 [P] Add configuration tests for strict previous-key JSON format/count/version/length, rotation-aware key lookup exposure to repair and throttle consumers, operation retention, online/repair-batch assignment limits, identifier lease/reconciliation settings, issuer/audience, access TTL range 1..900, refresh TTL greater than access and no more than 2592000, fixed 30-second refresh-rotation lease plus 60-second/100-item reconciliation bounds, throttle defaults 5/20/900 and 30/300, positive throttle overrides, `AUTH_TRUSTED_PROXY_CIDRS` JSON default/IPv4/IPv6 validation, exact `AUTH_TRUSTED_BROWSER_ORIGINS` JSON canonicalization/local defaults/required production HTTPS values, wildcard/null/credentials/non-root-path/query/fragment/duplicate/non-HTTPS production rejection, unsafe secrets, and redacted startup diagnostics in src/config/auth.config.spec.ts
- [x] T037 Implement production validation and rotation-aware loading for JWT/cookie/audit-correlation keys and previous-key JSON, exposing versioned key lookup without deriving repair or throttle keys, plus operation retention, online/repair-batch assignment limits, issuer/audience, bounded access/refresh TTLs, fixed refresh-rotation lease/reconciliation constants, throttle limit/window settings, exact `AUTH_TRUSTED_PROXY_CIDRS` JSON loading with empty default and production all-address rejection, canonical exact `AUTH_TRUSTED_BROWSER_ORIGINS` loading with localhost/127.0.0.1 non-production defaults and no production default, and identifier lease/reconciliation settings in src/config/auth.config.ts
- [x] T038 [P] Add AuthBrowserOriginGuard tests for exact configured origins; missing/null/multiple/malformed/wildcard-derived/untrusted denial; one generic 403; no cookie parsing, throttle/session/security-event lookup or mutation, or Set-Cookie; fixed route/reason dimensions with no raw origin/header/cookie/token/account data; at most one operational warning per dimension per minute per application instance; and suppressed-count carry-forward in src/auth/auth-browser-origin.guard.spec.ts
- [x] T039 Implement AuthBrowserOriginGuard as the pre-DTO/session boundary using canonical `AUTH_TRUSTED_BROWSER_ORIGINS`, exact single-Origin matching, generic denial, no persistent denial event, and fixed-cardinality per-instance operational warning sampling capped at one route/reason warning per minute with suppressed-count carry-forward in src/auth/auth-browser-origin.guard.ts
- [x] T040 [P] Add TokenSessionService tests for 900-second access ceiling, immutable family expiry no later than 30 days, remaining-lifetime cookie Max-Age, host-only HttpOnly production-Secure SameSite=Strict /auth set/clear parity, pending-marker insert failure, active 30-second lease denial, expired-lease takeover before CAS, family operation-id CAS, interruption after CAS, pending-to-committed-before-response ordering, 60-second/100-item scheduled reconciliation, committed/inconsistent pending revocation, multi-generation and concurrent replay safety, generic denials, and current/all-session sign-out in src/auth/token-session.service.spec.ts
- [x] T041 Implement immutable refresh-family expiry; hash-only pending/committed replay markers with unique rotation operation ids and fixed 30-second leases; family CAS that records lastRotationOperationId; expired pre-CAS takeover; request-time plus 60-second scheduled bounded reconciliation of at most 100 expired pending markers; orphaned post-CAS and invariant-failure revocation; committed-marker any-generation replay revocation; generic refresh denial; remaining-lifetime strict host-only cookie helpers; current/all-session revocation; and no runtime index creation; register RefreshTokenReplayMarker, refresh rotation reconciliation, and AuthBrowserOriginGuard in src/auth/token-session.service.ts and src/auth/auth.module.ts
- [x] T042 [P] Add AuthIdentifierRepairKeyPolicyService tests for indexed operation and unexpired throttle required-version queries, missing-key readiness with repair-key-required or throttle-key-required, worker and throttle denial without mutation, restoration recovery, two-previous-key rotation preflight, repair-key-rotation-blocked diagnostics, throttle rotation continuity, and safe version/count output in src/auth/auth-identifier-repair-key-policy.service.spec.ts
- [x] T043 Implement AuthIdentifierRepairKeyPolicyService as the single owner of required-version queries across incomplete repair operations and unexpired throttle buckets, missing-key readiness, repair-worker and throttle allow/deny decisions, and rotation preflight using the partial compound operation and throttle key-version indexes in src/auth/auth-identifier-repair-key-policy.service.ts
- [x] T044 Register and export AuthIdentifierRepairKeyPolicyService for reconciliation and health consumers in src/auth/auth.module.ts
- [x] T045 [P] Add process-level rotation preflight tests for stdin metadata validation, allowed/blocked boundaries across incomplete repair operations and unexpired throttle buckets, secret/unknown-field rejection, exact JSON limited to required versions/count plus fixed status/reason/maxPreviousKeys fields, exit codes 0/1/2, policy-service delegation, infrastructure failure, and no database/configuration/throttle mutation in test/auth-audit-key-rotation-preflight.e2e-spec.ts
- [x] T046 Implement the local audit-key rotation preflight adapter with validated stdin metadata, AuthIdentifierRepairKeyPolicyService delegation through a minimal application context, deterministic JSON limited to required versions/count plus fixed status/reason/maxPreviousKeys fields, exit codes 0/1/2, guaranteed context cleanup, and npm script wiring in scripts/preflight-auth-audit-key-rotation.ts and package.json
- [x] T047 [P] Add reconciliation unit tests for reservation discovery by pendingOperationId, valid recoverable/finalizing transitions, audit-before-TTL crash recovery, idempotent terminal events, gated-parent completion checks, bounded gate/batch cleanup before TTL, atomic lease acquisition, renewal/expiry, skew tolerance, multi-instance exclusion, clean terminal-operation ignore, and HMAC redaction in src/auth/auth-identifier-reconciliation.service.spec.ts
- [x] T048 [P] Add recovery e2e tests for migration-before-start, migration-required readiness, startup/scheduled reconciliation, bounded batches, competing workers/claims, reservation-reference repair, failed-retryable recovery, saga compensation, finalizing audit persistence, activation-gate fail-closed/unlock/cleanup behavior, shared key-policy repair-key-required no-mutation/restoration recovery, and non-TTL incomplete-cleanup operations in test/auth-identifier-recovery.e2e-spec.ts
- [x] T049 Implement idempotent post-migration startup/scheduled reconciliation with pending-reservation discovery, recoverable/finalizing state handling, foundational terminal-event persistence before TTL eligibility, AuthIdentifierRepairKeyPolicyService worker decisions, MongoDB-time leases, renewal, bounded activation-gate/repair-batch cleanup, clean terminal ignore, and multi-instance ownership in src/auth/auth-identifier-reconciliation.service.ts and src/auth/auth.module.ts
- [x] T050 Add auth-identifiers:read/manage permissions to the stable permission enum, administrator mapping, and normalized auth context in src/common/enums/auth-permission.enum.ts and src/auth/permissions.service.ts
- [x] T051 [P] Add guard tests proving explicit public metadata bypasses authentication while unmarked routes remain denied by default in src/auth/jwt-auth.guard.spec.ts and src/auth/permissions.guard.spec.ts
- [x] T052 Implement an explicit Public decorator and make JwtAuthGuard and PermissionsGuard honor it in src/auth/public.decorator.ts, src/auth/jwt-auth.guard.ts, and src/auth/permissions.guard.ts
- [x] T053 [P] Add unauthenticated liveness/readiness e2e tests for healthy state, runtime MongoDB/auth-infrastructure failure, AuthIdentifierRepairKeyPolicyService referenced-key removal/restoration with repair-key-required or throttle-key-required response, five-second response, and connection-string/secret/stack/host/account redaction in test/health.e2e-spec.ts
- [x] T054 Extend the existing health module with explicitly public GET /health liveness and public GET /health/ready runtime MongoDB/auth-infrastructure checks plus AuthIdentifierRepairKeyPolicyService readiness evaluation, returning redacted repair-key-required or throttle-key-required without mutating repair or throttle state, in src/health/health.controller.ts, src/health/health.service.ts, and src/health/health.module.ts
- [x] T055 Add direct `proxy-addr` and TypeScript type dependencies, configure the Express trust-proxy callback from validated `AUTH_TRUSTED_PROXY_CIDRS`, drive credentialed CORS from canonical `AUTH_TRUSTED_BROWSER_ORIGINS` with exact matched origins and no wildcard/suffix behavior, and wire the foundational auth module without a generic throttler guard in package.json, src/main.ts, and src/app.module.ts
- [x] T056 [P] Add AuthThrottleService unit tests for atomic 5/20/30 boundaries, every normalizable generic sign-in failure category, malformed source-only counting, shared compatibility-route keys, concurrent and multi-instance increments, automatic 900/300-second recovery, current/previous audit-key bucket continuity, missing-key fail-closed behavior without writes, direct/trusted/untrusted IPv4/IPv6 proxy chains, generic decisions, and absence of raw identifiers/source addresses/family ids/tokens in src/auth/auth-throttle.service.spec.ts
- [x] T057 Add AuthThrottleBucket schema with dimension, audit-correlation keyVersion, purpose-separated HMAC bucketKey, count, windowStartedAt, expiresAt, unique dimension/keyVersion/bucketKey index, and TTL/required-version index metadata in src/auth/schemas/auth-throttle-bucket.schema.ts
- [x] T058 Implement atomic MongoDB-backed AuthThrottleService counters that derive current and configured previous-version keys, continue matching unexpired buckets under their original keyVersion, create only with the current version, and fail closed with throttle-key-required when a referenced key is unavailable; implement direct-peer identity by default and `proxy-addr` right-to-left first-untrusted resolution for configured IPv4/IPv6 CIDRs in src/auth/auth-throttle.service.ts and src/auth/auth-source-identity.service.ts
- [x] T059 Register AuthThrottleBucket without runtime index creation and export AuthThrottleService/AuthSourceIdentityService as the only login and refresh throttling enforcement boundary in src/auth/auth.module.ts
- [x] T060 [P] Add e2e abuse tests proving every generic failure with a normalizable identifier increments the identifier bucket, malformed requests increment source only, the sixth identifier-correlated failure, twenty-first source sign-in attempt, and thirty-first refresh family/source attempt return the same generic 429 across shared/compatibility routes, counters are shared across application instances, current/previous key rotation preserves unexpired windows, missing referenced keys fail closed without writes, spoofed untrusted forwarding is ignored, trusted IPv4/IPv6 chains resolve to the first untrusted hop, windows recover after 900/300 seconds, and throttle documents contain no raw identifier/source/family/token values in test/auth.e2e-spec.ts
- [x] T061 [P] Add browser-session e2e tests across shared/compatibility login, refresh, logout, and logout-all for every configured exact origin; generic missing/null/multiple/malformed/untrusted 403 before cookie/throttle/session/security-event effects; bounded redacted per-instance operational warning sampling; strict host-only set/clear cookies; 900-second access ceiling; immutable 30-day family expiry; pending-marker interruption recovery; multi-generation/concurrent replay safety; and current/all-session revocation in test/auth-browser-session-security.e2e-spec.ts
- [x] T062 Apply AuthBrowserOriginGuard before DTO/cookie/session handling on shared login, compatibility login, refresh, logout, and logout-all; apply login/refresh precheck/increment policies directly through AuthThrottleService for every generic failure category, using identifier plus source when an identifier normalizes and source only for malformed requests, with generic origin/retry responses and shared bucket identities in src/auth/auth.controller.ts and src/auth/auth.module.ts
...
- [x] T078 [P] [US1] Add AuthService tests for reservation-based shared identifier resolution, staff/member success, completed activation-gate acceptance, missing/non-terminal/failed gate fail-closed behavior, no-role authentication with no protected access, legacy ambiguity denial, generic failures, opaque account references only after exact-one resolution, HMAC-only unresolved/ambiguous correlation, and redacted audit context in src/auth/auth.service.spec.ts
- [x] T079 [P] [US1] Add API e2e tests for POST /auth/login with staff, admin, and member credentials plus ambiguous identifiers and compatibility-wrapper parity in test/auth.e2e-spec.ts
- [x] T080 [P] [US1] Add frontend API tests for the unified login request, discriminated staff/member response, and generic failure behavior in frontend/src/lib/api/auth.test.ts
- [x] T081 [P] [US1] Add frontend tests for one shared sign-in form, roleArea/permission-based landing, accessible names, announced errors, deterministic failure focus, pending-state duplicate prevention, protected-route redirects, and generic errors in frontend/src/features/auth/shared-login.test.tsx
- [x] T082 [P] [US1] Add Playwright coverage for keyboard-only staff/admin/member shared sign-in, accessible errors, legacy-login redirects, and sign-out return behavior in frontend/tests/e2e/shared-sign-in.spec.ts and frontend/tests/e2e/accessibility.spec.ts
- [x] T083 [P] [US1] Add restart-persistence e2e coverage that recreates the Nest application against the same database and verifies staff/member roles, scope, ownership, and refresh continuity in test/auth-persistence.e2e-spec.ts
- [x] T084 [US1] Add a validated shared login request DTO and discriminated staff/member response types in src/auth/dto/shared-login.dto.ts
- [x] T085 [US1] Implement AuthIdentifier reservation lookup, exact-one-context enforcement, activationGateOperationId parent-completion verification with fail-closed missing/expired/non-terminal/failed behavior, legacy conflict denial, generic credential denial, no-role context handling, opaque failed-sign-in account references only after exact-one resolution, and HMAC-only redacted unresolved/ambiguous events in src/auth/auth.service.ts
- [x] T086 [US1] Add POST /auth/login behind the foundational AuthBrowserOriginGuard and make POST /auth/staff-login and POST /auth/member-login delegate to the same exact-origin, token, strict-cookie, AuthThrottleService, and audit behavior in src/auth/auth.controller.ts
- [x] T087 [US1] Consolidate frontend login types and API calls around POST /auth/login while retaining compatibility exports only where still required in frontend/src/lib/api/types.ts, frontend/src/lib/api/auth.ts, and frontend/src/lib/api/member-auth.ts
- [x] T088 [US1] Replace role-specific sign-in screens with one keyboard-accessible shared page with announced errors, deterministic focus and duplicate-submit prevention; route from roleArea/permissions, redirect legacy URLs, keep access credentials only in runtime memory, and make current/all-session sign-out clear memory/query caches and return to the shared route in frontend/src/features/auth/shared-login.tsx, frontend/src/app/router.tsx, frontend/src/lib/auth/session.ts, and frontend/src/lib/auth/sign-out.ts
...
- [x] T104 [P] [US3] Add e2e tests for admin staff-user create/update/deactivate/role assignment and regular-staff denial in test/auth.e2e-spec.ts
- [x] T105 [P] [US3] Add service tests for role validation, transactional identifier reservation, cross-context collision rejection, approved role assignment, authVersion bump, and session revocation in src/staff-users/staff-users.service.spec.ts
- [x] T106 [P] [US3] Add frontend tests for staff-user role assignment forms and access-denied states in frontend/src/features/auth/staff-role-management.test.tsx
- [x] T107 [P] [US3] Add Playwright acceptance test that an administrator can create a staff account, assign a role, sign in as that staff user, and confirm permitted access within 5 minutes in frontend/tests/e2e/staff-role-management.spec.ts
- [x] T108 [P] [US3] Add e2e tests for transaction/saga conflict resolution, explicit operation transitions, failed-retryable recovery, finalizing audit-before-TTL ordering, retained completed/failed-terminal replay with original HTTP status, retained/no-retained subjects, reservation-reference recovery, assignment limits 19/20/21 with 422 response, replacement collision, expiry requiring a new operation id, compensation, unchanged passwords, redaction, and foundational idempotent security events in test/auth-identifier-conflicts.e2e-spec.ts
- [x] T109 [P] [US3] Add deterministic manifest utility tests for normalized RFC 8785 canonical bytes, fixed HKDF salt/info/output length, published derivation/HMAC vectors, current/previous key selection, constant-time comparison, changed mappings, and unavailable key versions in src/auth/auth-identifier-repair-manifest.spec.ts
- [x] T110 [P] [US3] Add AuthIdentifierRepairAuthorizationService tests for dry-run secrecy, token signature/issuer/audience/expiry, current account/authVersion/role/permission revalidation before every mutating boundary, expired invocation pause, and redacted denial in src/auth/auth-identifier-repair-authorization.service.spec.ts
- [x] T111 [P] [US3] Add AuthIdentifierRepairService parent-lifecycle tests for manifest verification, delegated key-policy decisions, different-admin resume attribution/event, and pending/applying/failed-retryable/compensating/finalizing/terminal transitions in src/auth/auth-identifier-repair.service.spec.ts
- [x] T112 [US3] Add AuthIdentifierRepairService batch tests for unique bounded checkpoints, idempotent preparation/resume, reverse compensation, bounded gated activation, fail-closed authentication before parent completion, atomic parent unlock, and bounded gate/batch cleanup before TTL in src/auth/auth-identifier-repair.service.spec.ts
- [x] T113 [P] [US3] Add offline repair integration tests for oversized migration quarantine, transaction requirement, history preservation, one idempotent parent terminal event, restart recovery, and no data mutation while the manifest key is unavailable in test/auth-identifier-offline-repair.e2e-spec.ts
- [x] T114 [P] [US3] Add process-level CLI tests for token-through-stdin, actor-override rejection, dry-run output, confirmation refusal, service delegation, resumable exit codes, and token/identifier redaction in test/auth-identifier-offline-repair-cli.e2e-spec.ts
- [x] T115 [P] [US3] Add frontend tests for conflict listing, safe account labels, replacement validation, successful resolution, and forbidden states in frontend/src/features/auth/identifier-conflicts.test.tsx

### Implementation for User Story 3

- [x] T116 [US3] Update staff-user DTOs to expose permissions, validate approved roles, and redact credential fields in src/staff-users/dto/staff-user.dto.ts
- [x] T117 [US3] Update StaffUsersService to create/update/deactivate staff users with transactional AuthIdentifier reserve/release, validate roles, hash passwords, bump authVersion, revoke sessions, and record role/account security events in src/staff-users/staff-users.service.ts
- [x] T118 [US3] Update StaffUsersController to require staff-users:read, staff-users:manage, roles:read, and roles:manage permissions per route in src/staff-users/staff-users.controller.ts
- [x] T119 [US3] Add a role/permission review endpoint or service method for admin review in src/auth/auth.controller.ts and src/auth/permissions.service.ts
- [x] T120 [US3] Update frontend staff-user API types and mutations for permissions, role assignment, deactivation, and role review in frontend/src/lib/api/types.ts and frontend/src/lib/api/mutations.ts
- [x] T121 [US3] Add or update staff-user management UI for role assignment, deactivation, and permission review in frontend/src/features/auth/staff-role-management.tsx
- [x] T122 [US3] Update member identifier-changing workflows to reserve/release AuthIdentifier transactionally and record safe conflict events in src/members/members.service.ts
- [x] T123 [US3] Add deterministic conflict DTOs plus administrator resolve/status endpoints for transaction/saga execution, explicit recoverable/finalizing/terminal states, assignment-limit 422 validation, complete claimant accounting, pending/gated reservation references, compensation, retained completed/failed-terminal replay with original HTTP status, retention-scoped idempotency, and redacted results in src/auth/dto/auth-identifier.dto.ts, src/auth/auth.controller.ts, and src/auth/auth-identifier.service.ts
- [x] T124 [US3] Add frontend conflict/operation-status API types, queries, polling, and resolution mutation in frontend/src/lib/api/types.ts and frontend/src/lib/api/auth.ts
- [x] T125 [US3] Add administrator conflict review/resolution UI with pending, applying, compensating, finalizing, failed-retryable, completed, failed-terminal, expired, and manual-repair-required states in frontend/src/features/auth/identifier-conflicts.tsx and frontend/src/app/router.tsx
- [x] T126 [US3] Implement normalized RFC 8785 repair-manifest canonicalization and deterministic HKDF-SHA-256/HMAC derivation using fixed salt, versioned info, 32-byte output, current/previous key lookup, and constant-time comparison in src/auth/auth-identifier-repair-manifest.ts
- [x] T127 [US3] Implement AuthIdentifierRepairAuthorizationService for validation of the token supplied by the CLI plus current account, authVersion, administrator role area, and auth-identifiers:manage revalidation before data access and every mutating boundary in src/auth/auth-identifier-repair-authorization.service.ts
- [x] T128 [US3] Implement AuthIdentifierRepairService dry-run parent creation, manifest verification, delegated AuthIdentifierRepairKeyPolicyService decisions, original/resuming actor persistence, resume events, and explicit parent lifecycle transitions using AuthIdentifierRepairAuthorizationService in src/auth/auth-identifier-repair.service.ts
- [x] T129 [US3] Implement AuthIdentifierRepairService bounded batch preparation, unique checkpoint resume, and reverse compensation while preserving conflict blocking in src/auth/auth-identifier-repair.service.ts
- [x] T130 [US3] Implement AuthIdentifierRepairService bounded activation gates, applying-to-finalizing transition, atomic parent completion/audit unlock, and post-completion cleanup coordination before TTL eligibility in src/auth/auth-identifier-repair.service.ts
- [x] T131 [US3] Register AuthIdentifierRepairAuthorizationService, AuthIdentifierRepairService, and their manifest/key dependencies in src/auth/auth.module.ts
- [x] T132 [US3] Add the thin offline repair CLI adapter for token/input through stdin, explicit mappings, dry-run, confirmation, service invocation, resumable exit codes, and redacted output in scripts/resolve-auth-identifier-conflict.ts
...
**Independent Test**: Trigger exact-one, unresolved, and ambiguous failed sign-in, denied member access, role change, identifier-conflict resolution/recovery, authorized repair resume with original/resuming actors, refresh replay, and sign-out events; then sign in as admin and verify exact-one failures use only an opaque account reference, unresolved/ambiguous failures use only versioned correlation, and all required records are visible, filterable, and redacted. Separately verify browser-origin denials create no security-activity record.
...
- [X] T141 [P] Add frontend Playwright coverage for staff/admin role boundaries and sign-out refresh revocation in frontend/tests/e2e/staff-auth-permissions.spec.ts
- [X] T142 [P] Add operational documentation for auth environment variables, 900/2592000 token lifetime ceilings and non-sliding family expiry, exact `AUTH_TRUSTED_BROWSER_ORIGINS` JSON/CORS/origin-guard behavior, bounded non-persistent origin-denial warning sampling, strict host-only cookie attributes, pending/committed replay-marker leases/reconciliation/indexes, forced legacy-family reauthentication migration and irreversible rollback behavior, any-generation replay response, current/all-session sign-out, shared throttle defaults/overrides, exact `AUTH_TRUSTED_PROXY_CIDRS` JSON behavior, direct/trusted/untrusted proxy examples, liveness/readiness usage, indexed repair-key-required and throttle-key-required diagnostics, audit-correlation key retention for incomplete repairs and unexpired throttle buckets, `npm run auth:key-rotation:preflight` stdin schema/fixed output/exit codes and repair-key-rotation-blocked handling, online conflict recovery, token-authorized offline repair through stdin, canonical manifest key rotation/retirement, bounded repair batches and activation-gate cleanup, terminal-result replay/expiry, performance verification, shared sign-in behavior, and future Keycloak migration notes in ./README.md
- [X] T143 [P] Add history-preservation regression coverage for role changes, deactivation/reactivation, identifier correction, borrowing records, actor references, and security events in test/auth-history-preservation.e2e-spec.ts
- [X] T144 [P] Add deterministic e2e coverage that role/authVersion changes affect the next protected request and always within 60 seconds in test/authorization.e2e-spec.ts
- [X] T145 [P] Add test-only equivalent unprotected/protected benchmark handlers and production AppModule route-graph/build exclusion assertions in test/performance/auth-benchmark.module.ts, test/performance/auth-benchmark.controller.ts, and test/performance/auth-benchmark-isolation.spec.ts
- [X] T146 Implement the production-build benchmark runner with dedicated seeded MongoDB, disabled logging, 100 warm-ups, 500 requests at concurrency 10, nearest-rank auth-boundary overhead p95, 10,000 security events, runtime/hardware metadata, npm command, and evidence output in scripts/verify-auth-performance.ts, package.json, and specs/003-auth-roles-permissions/evidence/auth-performance.md
- [X] T147 Prepare the moderated script, consent language, role-specific goals, timing/failure definitions, anonymized template, and production release gate in specs/003-auth-roles-permissions/evidence/shared-sign-in-usability.md
- [ ] T148 Have product owner or QA conduct the study with 8 members, 8 staff users, and 4 administrators and record only aggregate first-attempt and 30-second results in specs/003-auth-roles-permissions/evidence/shared-sign-in-usability.md
  - Blocked on human participation: the study package and release gate are prepared; automated agents must not fabricate aggregate results.
- [X] T149 Run and document backend verification commands from quickstart.md with npm test, npm run test:e2e, npm run verify:auth-performance, exact-origin/bounded-telemetry/cookie/lifetime/pending-rotation-recovery/legacy-session-revocation/multi-generation-replay/sign-out acceptance checks, default 5/20/30 throttle acceptance checks, and allowed/blocked `npm run auth:key-rotation:preflight` smoke inputs in specs/003-auth-roles-permissions/quickstart.md
- [X] T150 Run and document frontend verification commands from quickstart.md with npm run frontend:test and npm run frontend:test:e2e in specs/003-auth-roles-permissions/quickstart.md

...
The browser-session boundary is now explicit: access tokens expire within 15 minutes and remain in memory, refresh families have a non-sliding maximum 30-day lifetime, and every exchanged refresh token remains detectable through a recoverable hash-only replay marker until family expiry. Rotation uses leased `pending` markers and family-recorded operation ids so interruptions can be reconciled before a marker becomes `committed`; legacy active families without complete marker history are revoked during migration. All endpoints that issue, use, or clear the refresh cookie reject non-exact browser origins before cookie parsing or session mutation, using bounded redacted operational telemetry rather than persistent denial events. Production refresh cookies are host-only, HTTP-only, Secure, `SameSite=Strict`, limited to `/auth`, and use the remaining absolute family lifetime.
...
**Browser Session Design**: `AuthBrowserOriginGuard` is the mandatory pre-session boundary for shared/compatibility sign-in, refresh, current-session sign-out, and all-session sign-out. `AUTH_TRUSTED_BROWSER_ORIGINS` is a JSON array and the single source for both exact credentialed CORS responses and guard decisions. Configuration canonicalizes origins with the platform URL parser and rejects missing production configuration, non-HTTP(S) values, production HTTP, credentials, non-root paths, queries, fragments, `null`, wildcards, regexes, duplicate canonical values, and multiple/malformed `Origin` headers. The guard checks one exact origin before downstream code parses cookies, increments refresh-family counters, sets/clears cookies, or reads/mutates refresh records. A denial writes no security event, authentication record, or throttle record; it may increment only fixed-cardinality in-memory counters and emit at most one redacted operational warning per route/reason category per minute per application instance, with suppressed counts reported in the next permitted sample and no supplied origin/header, cookie, token, or account reference. `TokenSessionService` inserts a unique hash-only `RefreshTokenReplayMarker` in `pending` state with a rotation operation id and 30-second lease before atomically compare-and-swapping the family current hash and recording that operation id. It marks the marker `committed` before returning the replacement credential. After lease expiry, a pending marker may be taken over only when the family still has the presented current hash; if the family already records the pending operation id, or marker/family state cannot be reconciled, the family is revoked and the generic denial is returned. Every 60 seconds, each instance reconciles at most 100 expired pending markers to finalize and revoke orphaned or inconsistent rotations; pre-CAS pending work remains available for request takeover. A committed marker match revokes the family. Family `expiresAt` never changes after sign-in, while cookie `Max-Age` is recalculated from its remaining lifetime.
...
**Security Contract Testing**: Configuration tests cover access lifetime `1..900` seconds, refresh lifetime longer than access and no more than 30 days, exact trusted-origin JSON parsing/canonicalization, local-development defaults, required production HTTPS origins, and rejection of missing/empty production lists, wildcard, `null`, credentials, non-root path/query/fragment values, duplicates, and non-HTTPS production origins. Guard and e2e tests prove shared/compatibility sign-in, refresh, logout, and logout-all accept each configured exact origin; reject missing, opaque, multiple, malformed, and untrusted origins with one generic response; perform no cookie parsing, throttle mutation, session lookup/mutation, persistent security-event write, or `Set-Cookie` on rejection; and bound redacted operational warnings to one per fixed route/reason category per minute per application instance while accounting for suppressed occurrences. Session and migration tests cover 15-minute access expiry, absolute non-sliding 30-day family expiry, cookie attribute parity on set/clear, remaining-lifetime `Max-Age`, replay after multiple rotations, pending-marker insert failure, lease takeover before CAS, interruption after CAS, pending-to-committed ordering, concurrent rotation, invariant-failure revocation, forced revocation of all active legacy families before upgraded refresh traffic, current/all-session sign-out, memory-only frontend storage, and frontend cache clearing.
...
**Session Constraints**: `ACCESS_TOKEN_TTL_SECONDS` defaults to and cannot exceed 900. `REFRESH_TOKEN_TTL_SECONDS` defaults to and cannot exceed 2,592,000, must exceed the access lifetime, and establishes an immutable family expiry. Refresh rotation uses a fixed 30-second ownership lease and bounded reconciliation of at most 100 expired markers every 60 seconds per application instance. `AUTH_TRUSTED_BROWSER_ORIGINS` defaults outside production to `["http://localhost:5173","http://127.0.0.1:5173"]`; production has no default and requires a non-empty JSON array of exact HTTPS origins. Credential-bearing CORS never uses wildcard or suffix matching. Refresh cookies omit `Domain`, use path `/auth`, `HttpOnly`, production `Secure`, and `SameSite=Strict`; set and clear operations use identical scope/security attributes. No origin rejection path may read a refresh cookie or write authentication, throttle, or security-event state. Origin-denial telemetry uses fixed route/reason dimensions and per-instance one-minute sampling only; it never includes a supplied origin/header, cookie, token, or account reference. Refresh denials remain generic across malformed, pending-recovery, expired, replayed, revoked, inactive-account, and stale-authorization outcomes. The migration revokes every active legacy refresh family and clears its token hashes before enabling upgraded refresh handling because complete historical replay markers cannot be reconstructed.
...
- **Security and Privacy by Default**: PASS. Authentication is required for protected areas, member access is scoped to the member's own records, secrets are hashed/redacted, ambiguous identifiers fail closed, failed sign-in events never expose submitted identifiers, throttle keys are non-reversible, access/refresh lifetimes are bounded, recoverable marker state keeps exchanged refresh credentials replay-detectable, legacy sessions without complete history are revoked, browser credentials avoid persistent script-readable storage, and exact-origin checks run before session access without attacker-controlled persistent writes.

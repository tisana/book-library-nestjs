...
test('staff can create a borrowing and return it', async ({ page }) => {
  ...
  await page.route('http://localhost:3000/members/member-1/policy-status', (route) =>
    route.fulfill({
      json: { memberId: 'member-1', status: 'active', membershipTypeId: 'tier-1', maxActiveLoans: 3, activeLoanCount: 1, remainingAllowance: 2, eligibleByStatus: true, withinLimit: true, limitReached: false },
    }),
  );
  await page.route('http://localhost:3000/members**', (route) =>
    route.fulfill({
      json: [{ id: 'member-1', memberNumber: 'M-1001', fullName: 'Jane Reader', membershipTypeId: 'tier-1', status: 'active', activeLoanCount: 1 }],
    }),
  );
